...
public class GHJOperator extends JoinOperator {
    ...
    private void buildAndProbe(Partition leftPartition, Partition rightPartition) {
        ...
        for (Record probingRecord : probeRecords) {
            DataBox probingValue = probingRecord.getValue(probeColumnIndex);
            if (!map.containsKey(probingValue)) continue;
            for (Record r : map.get(probingValue)) {
                Record joinedRecord = r.concat(probingRecord);
